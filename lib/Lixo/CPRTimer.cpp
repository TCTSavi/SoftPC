...
//		printf("Tempo decorrido:%f,ref:%f\n",dftDuration,RTtmp->Rtimer->FTimer);
		if (dftDuration < RTtmp->Rtimer->FTimer){
			...
			switch (RTtmp->Rtimer->RT3MainR){
				case MainRAN0	: RTtmp->Rtimer->MainRA00(); 	break;	//Inicializa parametros principais do eixo
				case MainRAN1	: RTtmp->Rtimer->MainRA01(); 	break;	//Se erro=true ++,se erro=false Inicialização Ok,+2
				case MainRAN2	: RTtmp->Rtimer->MainRA02(); 	break;	//Se erro=true aguarda , se erro=false MainRA00
				case MainRAN3	: RTtmp->Rtimer->MainRA03(); 	break;	//Inicializa portas seriais
